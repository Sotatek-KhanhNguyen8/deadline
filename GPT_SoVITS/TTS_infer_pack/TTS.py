...
from GPT_SoVITS.file_utils import load_audio
...
_t2s_default_weights_path = parent_dir / "pretrained_models" / "s1bert25hz-2kh-longer-epoch=68e-step=50232.ckpt"
_vits__default_weights_path = parent_dir / "pretrained_models" / "s2G488k.pth"
...
class TTSA_Config:
    default_configs={
                "device": "cpu",
                "is_half": False,
                "t2s_weights_path": str(_t2s_default_weights_path),
                "vits_weights_path": str(_vits__default_weights_path),
                "cnhuhbert_base_path": str(_cnhuhbert_base_path),
                "bert_base_path": str(_bert_base_path),
                "flash_attn_enabled": True
            }
    configs:dict = None
...
class TTSA:
    def __init__(self, configs: Union[dict, str, TTSA_Config]):
        if isinstance(configs, TTSA_Config):
            self.configs = configs
        else:
            self.configs:TTSA_Config = TTSA_Config(configs)
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        print(f'-----------------[save config]-------------------init_vits_weights-')
        self.configs.save_configs()
        print(f"Loading VITS weights from {weights_path}")
        print(f"Loading VITS weights map location {self.configs.device}")
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        print(f'-----------------[save config]-------------------init_t2s_weights-')
        self.configs.save_configs()
        self.configs.hz = 50
        ...
        print('hehehe..')
        torch_model = Text2SemanticLightningModule(config, "****", is_train=False,
                                                 flash_attn_enabled=self.configs.flash_attn_enabled)
        # onnx_program = torch.onnx.dynamo_export(torch_model, dict_s1["weight"])
